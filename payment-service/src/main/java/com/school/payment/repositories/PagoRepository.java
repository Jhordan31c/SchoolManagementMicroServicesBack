...
public interface PagoRepository extends JpaRepository<Pago, Long> {
    ...
    // ❌ ANTES: @Query("SELECT p FROM Pago p WHERE p.alumno.id = ?1 ORDER BY p.fecha_vencimiento")
    ...
    // ❌ ANTES: @Query("SELECT p FROM Pago p WHERE p.alumno.id = ?1 AND EXTRACT(YEAR FROM p.fecha_vencimiento) = ?2")
